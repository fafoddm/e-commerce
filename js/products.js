...
function createCard(producto) {
  tarjeta += `
    <div class="col-lg-3 col-md-4 col-sm-6 mb-4" onclick="setProdID(${producto.id})" style="color: #161515;">
        <div class="card light2 mb-4 custom-shadow h-100 bg-light cursor-active">
            <img src="./${producto.image}" class="card-img-top" alt="${producto.name}">
            <div class="card-body light2" style="background-color: #f2f2f2;">
                <p class="card-text"> 
...
function filtrarPorRangoDePrecio() {
  ...
  if (precioMaximo != "" && precioMinimo != "") {
    objetosFiltrados = objeto_productos.filter(
      (element) => element.cost >= precioMinimo && element.cost <= precioMaximo
    );
  } else if (precioMinimo == "") {
    objetosFiltrados = objeto_productos.filter(
      (element) => element.cost <= precioMaximo
    );
  } else if (precioMaximo == "") {
    objetosFiltrados = objeto_productos.filter(
      (element) => element.cost >= precioMinimo
    );
  }
...
function barrraDeBusqueda() {
  ...
  objeto_productos.forEach((producto) => {
    ...
    if (
      productName.includes(valorBusqueda) ||
      productDescription.includes(valorBusqueda)
    ) {
      createCard(producto);
    }
  });
